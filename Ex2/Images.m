...
% TODO: implement this function
green_channel = image_double(:,:,2);
image_mark_green = green_channel >= 0.7;
...
image_masked(repmat(image_mark_green,[1,1,3])) = 0;
...
% TODO: implement this function
image_reshaped = [image_double(:,1:256,:);image_double(:,257:512,:)];
...
tmp_padded = padarray(input, [2,2]);
kernel = repmat(kernel, [1,1,3]);
tmp_padded = im2double(tmp_padded);

for row = 3 : (size(tmp_padded,1)-2)
    for column = 3 : (size(tmp_padded,2)-2)
        tmp_array = (tmp_padded(row-2:row+2, column-2:column+2)).*kernel;
        result(row,column,:) = sum(sum(tmp_array));
    end
end

result = result(3:(size(result,1)-2),3:(size(result,2)-2));
end
...
% TODO: implement this function
image_edge = imfilter(image_reshaped,[-1,-2,-1;0,0,0;1,2,1]);
